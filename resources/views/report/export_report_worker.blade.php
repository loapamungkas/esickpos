...
<head>
	<title>Laporan Pegawai ({{ date('d M, Y', strtotime($tgl_awal)) . ' - ' . date('d M, Y', strtotime($tgl_akhir))}})
	</title>
	<style type="text/css">
		html {
	...
	<div class="header">
		<table class="w-100">
			<tr>
				...
				<td class="text-right">
					<p class="text-20 txt-green font-bold">LAPORAN PEGAWAI</p>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td class="text-left text-12 txt-dark2" colspan="2">{{ date('d M, Y', strtotime($tgl_awal)) . ' - ' .
					date('d M, Y', strtotime($tgl_akhir))}}</td>
				@php
			...
			<tr>
				...
				<td colspan="2" class="pt-30">
					...
					<span class="txt-green d-block text-12 mb-1">{{ $pegawai->nama }}</span>
					<span class="d-block text-10">EMAIL</span>
					<span class="txt-green d-block text-12">{{ $pegawai->email }}</span>
				</td>
			</tr>
		</table>
	</div>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<span class="text-left d-block text-12 txt-dark">{{ $supply->kode_barang }}</span>
						</td>
						<td>
			...
			<tfoot>
				...
				<tr>
					<td class="text-14 pt-15 text-right">
						...
						<span class="txt-green font-bold">{{ $jml_aktv_pengeluaran }}</span>
					</td>
				</tr>
			</tfoot>
		</table>
		...
		<ul>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					...
					<tr>
						<td class="text-left text-12 txt-dark">{{ date('H:i', strtotime($transaction->created_at)) }}
						</td>
						<td>
						...
						<td>
							...
							<span class="txt-green text-12 d-block">Rp. {{ number_format($transaction->total,2,',','.')
								}}</span>
						</td>
						@php
						...
						<td>
							...
							<span class="txt-dark2 text-12 d-block">Rp. {{ number_format($transaction->bayar,2,',','.')
								}}</span>
						</td>
						<td>
							<span class="text-10 txt-light d-block">Kembali</span>
							<span class="txt-dark text-12 d-block">Rp. {{ number_format($transaction->kembali,2,',','.')
								}}</span>
						</td>
					</tr>
					@endforeach
		...
		<table class="w-100">
			<tfoot>
				...
				<tr>
					<td class="text-14 pt-15 text-right">
						<span class="mr-20">JUMLAH AKTIVITAS</span>
						<span class="txt-green font-bold">{{ $jml_aktv_pemasukan }}</span>
					</td>
				</tr>
			</tfoot>
		</table>
