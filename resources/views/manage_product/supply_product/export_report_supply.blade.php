...
<head>
	<title>Laporan Pengeluaran ({{ date('d M, Y', strtotime($tgl_awal)) . ' - ' . date('d M, Y',
		strtotime($tgl_akhir))}})</title>
	<style type="text/css">
		html {
	...
	<div class="header">
		<table class="w-100">
			<tr>
				...
				<td class="text-right">
					<p class="text-20 txt-green font-bold">LAPORAN PENGELUARAN</p>
				</td>
			</tr>
			<tr>
			...
			<table class="w-100 mb-4">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<span class="text-left text-12 txt-light">{{ $supply->kode_barang }}</span>
						</td>
						<td>
							<span class="text-left text-12 txt-dark2">{{ $supply->nama_barang }}</span>
						</td>
						<td class="text-left text-12 txt-dark2">{{ $supply->jumlah }}</td>
						<td class="text-left text-12">{{ $supply->nama }}</td>
					</tr>
					@endforeach
				</tbody>
			</table>
